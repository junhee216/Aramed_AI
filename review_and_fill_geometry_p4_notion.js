...
// 기하_2024학년도_현우진_드릴_P4 Notion 필드 검토 및 26, 27번 필드 채우기
...
function loadProblems() {
	const problemPath = path.join(
		...
		'기하_2024학년도_현우진_드릴', '기하_2024학년도_현우진_드릴_P4_문제_deepseek.json'
	);
...
function loadSolution() {
	const solutionPath = path.join(
		'C:', 'Users', 'a', 'Documents', 'MathPDF', 'organized', '현우진',
		'기하_2024학년도_현우진_드릴', '기하_2024학년도_현우진_드릴_P4_해설_deepseek_r1.md'
	);
...
function generateErrorScenarioImproved(problem, solution) {
	...
	if (problemType === '벡터') {
		...
		if (solution && solution.includes('일차결합')) {
			customScenario += '4. 벡터의 일차결합에서 계수 계산 실수\n';
		}
		if (solution && solution.includes('경유점')) {
			customScenario += '5. 벡터의 변형(경유점 이용)에서 시점과 종점을 잘못 파악\n';
		}
		if (solution && solution.includes('최대') || solution.includes('최소')) {
			customScenario += '6. 벡터의 크기가 일정하지 않을 때 최대/최소를 각의 크기만으로 판단\n';
		}
		if (solution && solution.includes('벡터방정식')) {
			customScenario += '7. 벡터방정식으로 나타내는 도형을 잘못 파악\n';
		}
		if (solution && solution.includes('원')) {
			customScenario += '8. 원과 벡터를 결합할 때 원의 중심과 반지름을 잘못 적용\n';
		}
	}
...
async function reviewAndFillGeometryP4() {
	const startTime = Date.now();
	console.log('='.repeat(80));
	console.log('기하_2024학년도_현우진_드릴_P4 Notion 필드 검토 및 26, 27번 필드 채우기');
	console.log('='.repeat(80));
	
	await logger.init();
	await logger.info('REVIEW_GEOMETRY_P4', '작업 시작');
	...
	try {
		...
		// P4 문제만 필터링
		const p4Pages = allPages.filter(page => {
			const 문제ID = extractPropertyValue(page.properties['문제ID']);
			return 문제ID && 문제ID.includes('P4');
		});
		
		console.log(`📋 Notion에서 P4 문제 ${p4Pages.length}개 발견\n`);
		await logger.info('REVIEW_GEOMETRY_P4', `P4 문제 ${p4Pages.length}개 발견`);
		...
		// 각 P4 문제 검토 및 업데이트
		for (let i = 0; i < p4Pages.length; i++) {
			const page = p4Pages[i];
			const 문제ID = extractPropertyValue(page.properties['문제ID']);
			const progress = `[${i + 1}/${p4Pages.length}]`;
			...
			try {
				...
				// 문제ID에서 인덱스 추출 (예: P4_13 -> 13, P4_02 -> 02)
				const indexMatch = 문제ID.match(/P4[_-]?(\d+)/);
				...
				if (needsUpdate && Object.keys(updateProps).length > 0) {
					...
					await logger.info('REVIEW_GEOMETRY_P4', `업데이트 완료: ${문제ID}`, { fields: updatedFields });
				} else if (!needsUpdate) {
				...
				await logger.error('REVIEW_GEOMETRY_P4', `처리 실패: ${문제ID}`, {
					error: error.message,
					code: error.code
				});
			}
		...
		console.log('[최종 결과]');
		console.log('='.repeat(80));
		console.log(`총 P4 문제 수: ${p4Pages.length}개`);
		...
		await logger.info('REVIEW_GEOMETRY_P4', '작업 완료', {
			total: p4Pages.length,
		...
		await logger.error('REVIEW_GEOMETRY_P4', '작업 실패', {
			error: error.message,
			code: error.code,
			elapsedTime: `${elapsedTime}초`
		});
