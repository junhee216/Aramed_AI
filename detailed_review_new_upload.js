// detailed_review_new_upload.js
// 새로 업로드된 미적분 데이터 상세 검토
...
function validateMathLogic(problem) {
	...
	if (problem.변형요소) {
		...
		if (jsonCheck.valid && jsonCheck.parsed) {
			...
			// 변형요소의 값들이 의미있는지 확인
			for (const [key, value] of Object.entries(jsonCheck.parsed)) {
				if (Array.isArray(value) && value.length === 0) {
					warnings.push(`변형요소 "${key}"의 배열이 비어있음`);
				}
			}
		}
	}
...
async function detailedReview() {
	console.log('='.repeat(70));
	console.log('[새로 업로드된 미적분 데이터 상세 검토]');
	console.log('='.repeat(70));
	...
	// 새로 업로드된 항목 필터링 (미적분_2025학년도_현우진_드릴_P2)
	const newItems = allPages.filter(page => {
		const props = page.properties;
		const problemId = getPropertyValue(props['문제ID'] || Object.values(props)[0]);
		return problemId && problemId.includes('미적분_2025학년도_현우진_드릴_P2');
	});
	
	if (newItems.length === 0) {
		console.log('⚠️  "미적분_2025학년도_현우진_드릴_P2"로 시작하는 항목을 찾을 수 없습니다.');
	...
	console.log(`[2단계] ${itemsToReview.length}개 항목 상세 검토 중...\n`);
	...
	if (totalErrors === 0) {
		...
		if (results.구조오류.length > 0) {
			console.log(`[구조 오류] ${results.구조오류.length}개`);
			console.log('-'.repeat(70));
			results.구조오류.forEach(err => {
