// review_and_fill_geometry_p2_notion.js
// 기하_2024학년도_현우진_드릴_P2 Notion 필드 검토 및 26, 27번 필드 채우기
...
function loadProblems() {
	const problemPath = path.join(
		...
		'기하_2024학년도_현우진_드릴', '기하_2024학년도_현우진_드릴_P2_문제_deepseek.json'
	);
...
function loadSolution() {
	const solutionPath = path.join(
		'C:', 'Users', 'a', 'Documents', 'MathPDF', 'organized', '현우진',
		'기하_2024학년도_현우진_드릴', '기하_2024학년도_현우진_드릴_P2_해설_deepseek_r1.md'
	);
...
function generatePrincipleSharedProblems(problem, allProblems, solution) {
	...
	if (solution) {
		if (problemType === '타원' && solution.includes('타원')) {
			...
			if (solution.includes('초점을 지나는')) {
				solutionPrinciples.push('타원의 초점을 지나는 직선 문제');
			}
			if (solution.includes('중점')) {
				solutionPrinciples.push('타원의 중심과 중점을 이용하는 문제');
			}
		}
		if (problemType === '쌍곡선' && solution.includes('쌍곡선')) {
			...
			if (solution.includes('주축')) {
				solutionPrinciples.push('쌍곡선의 주축을 이용하는 문제');
			}
		}
		if (solution.includes('직각삼각형') && solution.includes('닮음')) {
			solutionPrinciples.push('직각삼각형의 닮음을 이용하는 문제');
		}
		if (solution.includes('삼각형') && solution.includes('합동')) {
			solutionPrinciples.push('삼각형의 합동을 이용하는 문제');
		}
	}
...
function generateErrorScenarioImproved(problem, solution) {
	...
	if (problemType === '타원') {
		...
		if (solution && solution.includes('대칭')) {
			customScenario += '4. 타원의 대칭성을 놓쳐서 중복 계산하거나 누락\n';
		}
		if (solution && solution.includes('중점')) {
			customScenario += '5. 타원의 중심과 중점의 관계를 잘못 파악\n';
		}
	}
	...
	if (problemType === '쌍곡선') {
		...
		if (solution && solution.includes('점근선')) {
			customScenario += '4. 쌍곡선의 점근선 방정식을 잘못 적용: y = ±(b/a)x를 y = ±(a/b)x로 착각\n';
		}
		if (solution && solution.includes('주축')) {
			customScenario += '5. 주축의 길이 2a와 반주축의 길이 a를 혼동\n';
		}
	}
	...
	if (solution && solution.includes('삼각형') && solution.includes('합동')) {
		customScenario += `${customScenario.split('\n').length}. 삼각형의 합동 조건을 잘못 적용\n`;
	}
	if (solution && solution.includes('평행선')) {
		customScenario += `${customScenario.split('\n').length}. 평행선에서 동위각, 엇각, 맞꼭지각을 혼동\n`;
	}
...
async function reviewAndFillGeometryP2() {
	const startTime = Date.now();
	console.log('='.repeat(80));
	console.log('기하_2024학년도_현우진_드릴_P2 Notion 필드 검토 및 26, 27번 필드 채우기');
	console.log('='.repeat(80));
	
	await logger.init();
	await logger.info('REVIEW_GEOMETRY_P2', '작업 시작');
	...
	try {
		...
		// P2 문제만 필터링
		const p2Pages = allPages.filter(page => {
			const 문제ID = extractPropertyValue(page.properties['문제ID']);
			return 문제ID && 문제ID.includes('P2');
		});
		
		console.log(`📋 Notion에서 P2 문제 ${p2Pages.length}개 발견\n`);
		await logger.info('REVIEW_GEOMETRY_P2', `P2 문제 ${p2Pages.length}개 발견`);
		...
		// 각 P2 문제 검토 및 업데이트
		for (let i = 0; i < p2Pages.length; i++) {
			const page = p2Pages[i];
			const 문제ID = extractPropertyValue(page.properties['문제ID']);
			const progress = `[${i + 1}/${p2Pages.length}]`;
			...
			try {
				...
				// 문제ID에서 인덱스 추출 (예: P2_01 -> 01)
				const indexMatch = 문제ID.match(/P2[_-]?(\d+)/);
				...
				if (needsUpdate && Object.keys(updateProps).length > 0) {
					...
					await logger.info('REVIEW_GEOMETRY_P2', `업데이트 완료: ${문제ID}`, { fields: updatedFields });
				} else if (!needsUpdate) {
				...
				await logger.error('REVIEW_GEOMETRY_P2', `처리 실패: ${문제ID}`, {
					error: error.message,
					code: error.code
				});
			}
		...
		console.log('[최종 결과]');
		console.log('='.repeat(80));
		console.log(`총 P2 문제 수: ${p2Pages.length}개`);
		...
		await logger.info('REVIEW_GEOMETRY_P2', '작업 완료', {
			total: p2Pages.length,
		...
		await logger.error('REVIEW_GEOMETRY_P2', '작업 실패', {
			error: error.message,
			code: error.code,
			elapsedTime: `${elapsedTime}초`
		});
