// check_p8_json_errors.js
// P8 JSON 오류 항목 상세 확인 및 수정
...
async function checkAndFixP8Errors() {
	console.log('='.repeat(70));
	console.log('[P8 JSON 오류 항목 상세 확인 및 수정]');
	console.log('='.repeat(70));
	...
	const response = await notion.databases.query({
		...
		filter: {
			property: '문제ID',
			title: {
				contains: '미적분_2025학년도_현우진_드릴_P8'
			}
		},
		page_size: 100,
	});
	...
	const errorItems = items.filter(page => {
		const props = page.properties;
		const problemId = getPropertyValue(props['문제ID']);
		return problemId.includes('P8_04') || problemId.includes('P8_06');
	});
	
	console.log(`\n총 ${errorItems.length}개 오류 항목 발견\n`);
	
	for (let i = 0; i < errorItems.length; i++) {
		const page = errorItems[i];
		const props = page.properties;
		const problemId = getPropertyValue(props['문제ID']);
