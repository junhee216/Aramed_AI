# convert_haktong_p7_solution_deepseek_final.py
# 확통_2024학년도_현우진_드릴_P7_해설 LaTeX → Deepseek R1-70B용 마크다운 변환 (최종 버전)
...
def latex_to_markdown_improved(latex_text):
    """LaTeX를 Deepseek R1-70B용 마크다운으로 변환 (개선 버전)"""
    text = latex_text
    
    # 수식 블록 ($$ ... $$) - 먼저 처리 (가장 긴 매치 우선)
...
def extract_solutions(latex_content):
    ...
    # 섹션별로 분리 (수식이 포함된 제목도 처리)
    # 섹션 제목에 수식이 있을 수 있으므로 더 정확한 패턴 사용
    sections = re.split(r'\\section\*\{', body)
    
    for i, section in enumerate(sections[1:], 1):  # 첫 번째는 빈 문자열
        # 제목과 내용 분리
        ...
        # 다음 섹션 시작 전까지가 내용
        next_section = re.search(r'\\section\*\{', content)
        if next_section:
...
def main():
    print("=" * 80)
    print("확통_2024학년도_현우진_드릴_P7_해설 → Deepseek R1-70B용 변환 (최종)")
    print("=" * 80)
    ...
    for sol in solutions:
        markdown_content = latex_to_markdown_improved(sol['content'])
