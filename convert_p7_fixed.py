# convert_p7_fixed.py
# 미적분 드릴 P7 문제 LaTeX를 딥시크용 CSV로 변환 (수정 버전)
...
def extract_problems_manual(latex_content):
    ...
    # 문제 1: $x \geq 0$ 에서 정의된 함수 (주관식)
    p1_match = re.search(r'(\$x \\geq 0\$.*?구하시오\. \[4점\])', body, re.DOTALL)
    if p1_match:
    ...
    # 문제 2: 함수 $f(x)=x+...$ (주관식)
    p2_match = re.search(r'(함수 \$f\(x\)=x\+.*?구하시오\. \[4점\])', body, re.DOTALL)
    if p2_match:
    ...
    # 문제 3: 함수 $f(x)=3 x^{3}+...$ (객관식)
    p3_match = re.search(r'(함수 \$f\(x\)=3 x\^\{3\}.*?\[4점\])(.*?)(?:\(1\)|\\section)', body, re.DOTALL)
    if p3_match:
    ...
    # 문제 4: 최고차항의 계수가 1 인 이차함수 (주관식)
    p4_match = re.search(r'(최고차항의 계수가 1 인 이차함수.*?구하시오\. \[4점\])', body, re.DOTALL)
    if p4_match:
    ...
    # 문제 5: 구간 $[0, \infty)$ ... (객관식)
    p5_match = re.search(r'(구간 \$\[0, \\infty\)\$.*?\[4점\])(.*?)(?:\(1\)|\\section)', body, re.DOTALL)
    if p5_match:
    ...
    # 문제 6: 닫힌구간 $[0, \pi]$ ... (객관식)
    p6_match = re.search(r'(닫힌구간 \$\[0, \\pi\]\$.*?\[3점\])(.*?)(?:\(1\)|\\end)', body, re.DOTALL)
    if p6_match:
