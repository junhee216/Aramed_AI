# convert_p8_problems_latex.py
# 미적분 드릴 P8 문제 LaTeX를 딥시크용 CSV로 변환
...
def extract_problems_from_latex(latex_content):
    ...
    # 문제별로 정확히 추출
    problem_patterns = [
        (r'(함수 \$f\(x\)=x\^\{3\}-12 x\^\{2\}\+36 x\$.*?구하시오\. \[4점\])', 1, 4, False),
        (r'(두 곡선 \$y=\\ln.*?\[3점\])(.*?)(?:\(1\)|\\section)', 2, 3, True),
        (r'(함수 \$f\(x\)=\\int.*?\[4점\])(.*?)(?:\(1\)|실수)', 3, 4, True),
        (r'(실수 전체의 집합에서 미분가능하고.*?구하시오\. \[4점\])', 4, 4, False),
        (r'(\$0<x \\leq 1\$.*?구하시오\. \[4점\])', 5, 4, False),
        (r'(함수 \$f\(x\)=x\^\{2\}-3 x\+2 \\ln x\+4\$.*?\[4점\])(.*?)(?:\(1\)|\\end)', 6, 4, True),
    ]
    ...
    for pattern, index, point, has_options in problem_patterns:
        ...
        if match:
            ...
            options_text = match.group(2) if match.lastindex >= 2 and has_options else ""
