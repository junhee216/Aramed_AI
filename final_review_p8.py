# final_review_p8.py
# 미적분 드릴 P8 문제 LaTeX를 딥시크용 CSV로 변환 (최종 검토)
...
def extract_problems_correct(latex_content):
    """정확한 문제 추출"""
    problems = []
    ...
    # 문제 1: 함수 $f(x)=x^{3}-12 x^{2}+36 x$ (주관식, [4점])
    # 섹션 16 다음에 나오는 첫 번째 문제
    p1_pattern = r'(함수 \$f\(x\)=x\^\{3\}-12 x\^\{2\}\+36 x\$.*?구하시오\. \[4점\])'
    ...
    # 문제 2: 두 곡선 (객관식, [3점])
    p2_pattern = r'(두 곡선.*?\[3점\])(.*?)(?=\\section|\\end|실수 전체)'
    ...
    # 문제 3: 함수 $f(x)=\int \cos (\pi x^{2}) d x$ (객관식, [4점])
    p3_pattern = r'(함수 \$f\(x\)=\\int.*?\[4점\])(.*?)(?=실수 전체|\\section|\\end)'
    p3_match = re.search(p3_pattern, body, re.DOTALL)
    if p3_match:
    ...
    # 문제 4: 실수 전체의 집합에서 미분가능하고 (주관식, [4점])
    p4_pattern = r'(실수 전체의 집합에서 미분가능하고.*?구하시오\. \[4점\])'
    p4_match = re.search(p4_pattern, body, re.DOTALL)
    if p4_match:
    ...
    # 문제 5: $0<x \leq 1$ 에서 정의된 함수 (주관식, [4점])
    p5_pattern = r'(\$0<x \\leq 1\$.*?구하시오\. \(단.*?\) \[4점\])'
    p5_match = re.search(p5_pattern, body, re.DOTALL)
    if not p5_match:
        # 단 조건 없는 경우
        p5_pattern = r'(\$0<x \\leq 1\$.*?구하시오\. \[4점\])'
        p5_match = re.search(p5_pattern, body, re.DOTALL)
    if p5_match:
    ...
    # 문제 6: 함수 $f(x)=x^{2}-3 x+2 \ln x+4$ (객관식, [4점])
    p6_pattern = r'(함수 \$f\(x\)=x\^\{2\}-3 x\+2 \\ln x\+4\$.*?\[4점\])(.*?)(?=\\end|$)'
...
def main():
    print("=" * 80)
    print("[미적분 드릴 P8 문제 LaTeX → CSV 변환 (최종 검토)]")
    print("=" * 80)
    ...
    # 문제 추출
    print("\n[2단계] 문제 추출 중...")
    problems = extract_problems_correct(latex_content)
