# convert_haktong_p7_solution_deepseek_perfect.py
# 확통_2024학년도_현우진_드릴_P7_해설 LaTeX → Deepseek R1-70B용 마크다운 변환 (완벽 버전)
...
def extract_title_with_math(text, start_pos):
    """수식이 포함된 섹션 제목 추출"""
    # \section*{ 부터 시작
    ...
    if depth == 0:
        title = text[pos + 1:i - 1]
        return title, i, pos - 9  # \section*{ 길이
    
    return None, None, None

def latex_to_markdown_perfect(latex_text):
    """LaTeX를 Deepseek R1-70B용 마크다운으로 완벽 변환"""
    text = latex_text
    ...
    def replace_display_math(match):
        ...
        # \left, \right 제거
        math_content = re.sub(r'\\left([\(\[\{])', r'\1', math_content)
        math_content = re.sub(r'\\right([\)\]\}])', r'\1', math_content)
...
def extract_solutions_perfect(latex_content):
    """해설 섹션 완벽 추출"""
    solutions = []
    ...
    pos = 0
    while True:
        section_match = re.search(r'\\section\*\{', body[pos:])
        if not section_match:
            break
        
        section_start = pos + section_match.start()
        title_start = section_start + len('\\section*{')
        ...
        if depth == 0:
            title = body[title_start:i-1].strip()
            
            if title != 'Comment':
                # 다음 섹션까지가 내용
                next_section = re.search(r'\\section\*\{', body[i:])
                if next_section:
                    content = body[i:next_section.start() + i].strip()
                else:
                    content = body[i:].strip()
...
def main():
    print("=" * 80)
    print("확통_2024학년도_현우진_드릴_P7_해설 → Deepseek R1-70B용 변환 (완벽 버전)")
    print("=" * 80)
    
    solutions = extract_solutions_perfect(latex_content)
    print(f"\n총 {len(solutions)}개 섹션 발견\n")
    
    markdown_solutions = []
    for sol in solutions:
        markdown_content = latex_to_markdown_perfect(sol['content'])
