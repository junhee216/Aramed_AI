# convert_p8_solution_latex.py
# 미적분 드릴 P8 해설 LaTeX를 딥시크용 CSV로 변환
...
def extract_solutions_from_latex(latex_content):
    ...
    p1_match = re.search(r'Drill 구분구적법으로 넓이 인식(.*?)(?=Drill|\\section|\\end)', body, re.DOTALL)
    if p1_match:
    ...
    p2_match = re.search(r'Drill 개형 파악이 어려운 두 곡선 사이의 넓이(.*?)(?=Drill|\\section|\\end)', body, re.DOTALL)
    if p2_match:
        content = p2_match.group(1).strip()
        # 개념 부분 추출
        concept_match = re.search(r'연속인 두 곡선.*?계산할 수 있다\.(.*?)(?=만약|앞의 문제)', content, re.DOTALL)
        if concept_match:
            concept_content = concept_match.group(1).strip()
            concept_content = re.sub(r'\\\\', ' ', concept_content)
            concept_content = re.sub(r'\\begin\{itemize\}.*?\\end\{itemize\}', '', concept_content, flags=re.DOTALL)
            concept_content = re.sub(r'\$\$.*?\$\$', '', concept_content, flags=re.DOTALL)
            concept_content = re.sub(r'\s+', ' ', concept_content)
            if len(concept_content) > 20:
                solutions.append({
                    "type": "concept",
                    "topic": "두 곡선 사이의 넓이",
        ...
        # 전략 부분 추출
        strategy_match = re.search(r'만약 교점의.*?부호 변화와 같다\.(.*?)(?=앞의 문제|$)', content, re.DOTALL)
        if strategy_match:
        ...
        # 문제 해설 부분
        problem_match = re.search(r'앞의 문제에서도.*?보인다\.', content, re.DOTALL)
        if problem_match:
            problem_content = problem_match.group(0).strip()
            problem_content = re.sub(r'\\\\', ' ', problem_content)
            problem_content = re.sub(r'\s+', ' ', problem_content)
            solutions.append({
                "type": "problem",
                "question_ref": 2,
    ...
    p3_match = re.search(r'Drill 일단 할 수 있는 것부터 해보기(.*?)(?=Drill|\\section|\\end)', body, re.DOTALL)
    if p3_match:
    ...
    p4_match = re.search(r'Drill 그래프의 대칭을 이용한 넓이의 관점으로(.*?)(?=Drill|\\section|\\end)', body, re.DOTALL)
    if p4_match:
    ...
    if p5_concept_match:
        content = p5_concept_match.group(1).strip()
        # 넓이의 관계 부분
        area_match = re.search(r'넓이의 관계.*?바꾸어 생각할 수 있다\.', content, re.DOTALL)
        if area_match:
            area_content = area_match.group(0).strip()
            area_content = re.sub(r'\\\\', ' ', area_content)
            area_content = re.sub(r'\\includegraphics.*?}', '', area_content)
            area_content = re.sub(r'\$\$.*?\$\$', '', area_content, flags=re.DOTALL)
            area_content = re.sub(r'\s+', ' ', area_content)
            if len(area_content) > 20:
                solutions.append({
                    "type": "concept",
...
def save_for_deepseek(solutions):
    ...
    with open(csv_path, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        for solution in solutions:
            ...
            content = solution.get('content', '') or solution.get('strategy', '')
...
def main():
    print("=" * 80)
    print("[미적분 드릴 P8 해설 LaTeX → CSV 변환]")
    print("=" * 80)
